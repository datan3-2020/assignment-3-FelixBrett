...
files <- dir(
             ...
             "C:/Users/F/Documents/Politics and International relations/Data Analysis in Social Science III/UKDA-6614-tab/tab",
...
```{r}
meanVote6_table <- meanVote6 %>%
  spread(wave, mean)
meanVote6_table

# short interpretation of findings:

# this table shows that women are consistently more  politically interested than men 
...
Long <- Long %>%
  select(pidp, wave, memorig, sex_dv, vote6)

# dissecting the age at wave 1 from the original all7 dataset to use later

all7_age <- all7 %>%
  select(pidp, a_age_dv)

# converting the Long dataset to the wide format and adding only the age
# at wave 1

interest <- Long %>%
      pivot_wider(names_from = wave, values_from = vote6) %>%
  left_join(all7_age, by = "pidp")

# removing all rows with incomplete waves (1)

interest <- na.omit(interest)

# calculating Delta for each individual (2)

interest <- interest %>% 
  mutate(Delta = abs(a - b) + abs(b - c) + abs(c - d) + abs(d - e) + abs(e - f) + abs(f - g))

# calculating mean Delta for men and women (3)

mean_Delta_sex <- interest %>%
group_by(sex_dv) %>%
  summarise(mean_Delta = mean(Delta, na.rm = TRUE))
mean_Delta_sex

# calculating mean Delta by age in years (4)

mean_Delta_age <- interest %>%
  group_by(a_age_dv) %>%
  summarise(mean_Delta = mean(Delta, na.rm = TRUE))
mean_Delta_age

# creating a graph to plot the relationship between age and mean_Delta

ggplot(data = mean_Delta_age) +
  geom_point(mapping = aes(x = a_age_dv, y = mean_Delta),
             col = "red")

# short interpretation of findings:

# there is almost no difference between men and women in how stable their
# involvement in politics is over the course of the study
# the difference between men and women in the Delta score is insignificant
# with mere 0.03 point difference between them (correct to 2 decimal places)

# whereas the graph indicates that political interest becomes less stable
